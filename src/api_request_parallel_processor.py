...
async def process_api_requests_from_file(
    ...
    with open(requests_filepath) as file:
            ...
            while True:
                ...
                if seconds_since_rate_limit_error < seconds_to_pause_after_rate_limit_error:
                    ...
                    logging.warn(
                        ...
                        f" {time.ctime(status_tracker.time_of_last_rate_limit_error + seconds_to_pause_after_rate_limit_error)}"
                    )
        ...
        if status_tracker.num_tasks_failed > 0:
            logging.warning(
                f"{status_tracker.num_tasks_failed} / {status_tracker.num_tasks_started} requests"
                f" failed. Errors logged to {save_filepath}."
            )
        if status_tracker.num_rate_limit_errors > 0:
            logging.warning(
                f"{status_tracker.num_rate_limit_errors} rate limit errors received. Consider"
    ...
    async def call_api(
        ...
        if error:
            ...
            if self.attempts_left:
                ...
                logging.error(
                    f"Request {self.request_json} failed after all attempts. Saving errors:"
                    f" {self.result}"
                )
                data = (
...
def num_tokens_consumed_from_request(
    ...
    elif api_endpoint in ("embeddings", "moderations"):
